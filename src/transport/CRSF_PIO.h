...
// PIO UART RX Program - Optimized for 8 cycles per bit
static uint16_t crsf_uart_rx_program_instructions[] = {
            //     .wrap_target
    0x2020, //  0: wait   0 pin, 0           ; Wait for start bit (0)
    0xea27, //  1: set    x, 7           [10]; Wait 11 cycles (1+10). At 8cyc/bit, this is 1.375 bits.
    0x4001, //  2: in     pins, 1            ; Sample 1 bit
    0x0642, //  3: jmp    x--, 2         [6] ; Wait 7 cycles (1+6) and loop. Total 8 cycles per bit.
    0x8020, //  4: push   block              ; Push byte to FIFO
            //     .wrap
};
...
public:
    ...
    static const uint32_t DMA_INIT_COUNT = 0xFFFFFFFF; 
    ...
    void begin(uint32_t pin, uint32_t baud) {
        ...
        gpio_pull_up(pin);        // Ensure pin is high when idle
        pio_gpio_init(_pio, pin); // Set pin function to PIO
        ...
        // Setup DMA for continuous ring buffer
        ...
        channel_config_set_ring(&dc, true, 8); // 2^8 = 256 bytes

        dma_channel_configure(
            _dma_chan, &dc,
            _rx_buffer,     // Dest
            &_pio->rxf[_sm], // Source
            DMA_INIT_COUNT, // "Infinite" transfers
            true            // Start
        );
    }
