...
public:
    ...
    bool processByte(uint8_t b, Frame &outFrame) {
        ...
        switch (_state) {
            ...
            case WAIT_PAYLOAD:
                if (_pos == _len + 2) {
                    uint8_t crc = _crsfCrc8(&_buffer[2], _len - 1);
                    if (crc == _buffer[_pos - 1]) {
                        if (_buffer[2] == CRSF_TYPE_RC_CHANNELS) {
                            _decodeChannels(_buffer + 3, outFrame.channels);
...
private:
    ...
    void _decodeChannels(const uint8_t *p, uint16_t *channels) {
